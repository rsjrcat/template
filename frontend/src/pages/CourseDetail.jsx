...
const CourseDetail = () => {
  ...
  const { data: course, loading, error, refetch } = useApi(
    () => coursesAPI.getCourseByCode(courseCode),
    [courseCode]
  );
  const { data: allCoursesData } = useApi(() => coursesAPI.getAllCourses());
  ...
  const handleShare = async () => {
    const shareData = {
      title: course?.courseName || 'BMS Academy Course',
      text: course?.description || 'Check out this course from BMS Academy',
    ...
    try {
      ...
      try {
        ...
        toast.error('Failed to share course link');
      }
    }
  };
  ...
  const relatedCourses = React.useMemo(() => {
    if (!allCoursesData || !course) return [];
    
    const allCourses = allCoursesData.flatMap(category => 
    ...
    return allCourses
      .filter(c => c.category === course.category && c.courseCode !== courseCode)
      .slice(0, 3);
  }, [allCoursesData, course, courseCode]);

  if (loading) {
    return (
      <div className="min-h-screen bg-gray-50 py-8">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          <LoadingSpinner size="lg" text="Loading course details..." className="py-20" />
        </div>
      </div>
    );
  }
            ...
            <div className="lg:col-span-1">
              <div className="bg-white rounded-lg shadow-xl p-6 sticky top-24">
                ...
                <div className="relative mb-6">
                  ...
                  <div 
                    className="w-full h-48 bg-gradient-to-r from-primary-500 to-primary-600 rounded-lg flex items-center justify-center"
              ...
              {relatedCourses.map((relatedCourse) => (
                ...
                >
                  <div className="h-48 bg-gradient-to-r from-primary-500 to-primary-600 relative">
                    ...
                    <div className="flex items-center justify-center h-full" style={{ display: relatedCourse.image ? 'none' : 'flex' }}>
